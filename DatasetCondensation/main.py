...
from utils import get_loops, get_dataset, get_network, get_eval_pool, evaluate_synset, get_daparam, match_loss, \
    get_time, TensorDataset, epoch, DiffAugment, ParamDiffAug, get_pcam_dataloaders, PCAMImageProvider
import wandb
...
def main():
    ...
    wandb.init(project="dds", name=args.method + '_' + args.dataset + '_' + args.model + '_' + str(
        args.ipc) + '_' + args.eval_mode + '_' + args.dsa_strategy + '_' + args.dis_metric + '_' + str(args.lr_img)+ '_ntrimg' + str(len(dst_train[0])))
    ...
    for exp in range(args.num_exp):
        ...
        image_provider = PCAMImageProvider(dst_train[0], dst_train[1], args.batch_real, device=args.device)
            ...
            for ol in range(args.outer_loop):
                ...
                for module in net.modules():
                    if 'BatchNorm' in module._get_name(): #BatchNorm
                        BN_flag = True
                if BN_flag:
                    img_real = torch.cat([image_provider.get_images(c)[:BNSizePC] for c in range(num_classes)], dim=0)
                ...
                for c in range(num_classes):
                    img_real = image_provider.get_images(c)
