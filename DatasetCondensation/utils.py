...
class PCAMClass(torchvision.datasets.PCAM):
    ...
    def __len__(self):
        return 4000#len(self.class_mapping)
...
def get_dataset(dataset, data_path, batch_size=256):
    if dataset == 'MNIST':
        ...
        mean = [0.70075601, 0.53835782, 0.69162057]
        std = [0.18174392, 0.20083655, 0.16479424]
        transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Resize((32, 32), antialias=True), transforms.Normalize(mean=mean, std=std)])
    ...
    elif dataset == 'PCAM32_SPLIT':
        channel = 3
        im_size = (32, 32)
        num_classes = 2
        mean = [178.69278044708753 / 255, 137.28123995951555 / 255, 176.36324185008846 / 255]
        std = [46.344700260152216 / 255, 51.21332066737447 / 255, 42.02253038386832 / 255]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32), antialias=True), transforms.Normalize(mean=mean, std=std)])
        y_0_idx = np.load('./data/pcam/splits/y_0_idx.npy')
        y_1_idx = np.load('./data/pcam/splits/y_1_idx.npy')
        dst_train = [PCAMClass(root='./data', split='val', class_mapping=y_0_idx, transform=transform),
                     PCAMClass(root='./data', split='val', class_mapping=y_1_idx,
                               transform=transform)]  # no augmentation
        dst_test = datasets.PCAM(data_path, split='test', download=True, transform=transform)
        class_names = [str(c) for c in range(num_classes)]

    elif dataset == 'PCAM32FULL_SPLIT':
        channel = 3
        im_size = (32, 32)
        num_classes = 2
        mean = [178.69278044708753 / 255, 137.28123995951555 / 255, 176.36324185008846 / 255]
        ...
        y_0_idx = np.load('./data/pcam/splits/y_0_idx.npy')
        y_1_idx = np.load('./data/pcam/splits/y_1_idx.npy')
        dst_train = [PCAMClass(root='./data', split='train', class_mapping=y_0_idx, transform=transform),
                     PCAMClass(root='./data', split='train', class_mapping=y_1_idx,
                               transform=transform)]  # no augmentation
        dst_test = datasets.PCAM(data_path, split='val', download=True, transform=transform)
        class_names = [str(c) for c in range(num_classes)]

    elif dataset == 'PCAM_SPLIT':
        channel = 3
        im_size = (96, 96)
        num_classes = 2
        mean = [178.69278044708753 / 255, 137.28123995951555 / 255, 176.36324185008846 / 255]
        ...
        y_1_idx = np.load('./data/pcam/splits/y_1_idx.npy')
        dst_train = [PCAMClass(root='./data', split='train', class_mapping=y_0_idx, transform=transform),
                    PCAMClass(root='./data', split='train', class_mapping=y_1_idx, transform=transform)]  # no augmentation
        dst_test = datasets.PCAM(data_path, split='val', download=True, transform=transform)
        class_names = [str(c) for c in range(num_classes)]
    elif dataset == 'PCAM_CC_SPLIT':
        channel = 3
        im_size = (32, 32)
        num_classes = 2
        mean = [178.69278044708753 / 255, 137.28123995951555 / 255, 176.36324185008846 / 255]
        std = [46.344700260152216 / 255, 51.21332066737447 / 255, 42.02253038386832 / 255]
        transform = transforms.Compose([transforms.ToTensor(), transforms.CenterCrop((32, 32)), transforms.Normalize(mean=mean, std=std)])
        y_0_idx = np.load('./data/pcam/splits/y_0_idx.npy')
        y_1_idx = np.load('./data/pcam/splits/y_1_idx.npy')
        dst_train = [PCAMClass(root='./data', split='val', class_mapping=y_0_idx, transform=transform),
                     PCAMClass(root='./data', split='val', class_mapping=y_1_idx,
    ...
    elif dataset == 'PCAM_CC_FULL_SPLIT':
        channel = 3
        im_size = (32, 32)
        num_classes = 2
        mean = [178.69278044708753 / 255, 137.28123995951555 / 255, 176.36324185008846 / 255]
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=batch_size, shuffle=False, num_workers=6)
...
def get_pcam_dataloaders(dataset_0, dataset_1, batch_size=256, num_workers=6):
    dataloader_0 = torch.utils.data.DataLoader(dataset_0, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    dataloader_1 = torch.utils.data.DataLoader(dataset_1, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    return dataloader_0, dataloader_1


class PCAMImageProvider():
    def __init__(self, dataset_0, dataset_1, batch_size=256, num_workers=6, device='cuda:0'):
        self.dataloader_0, self.dataloader_1 = get_pcam_dataloaders(dataset_0, dataset_1, batch_size=batch_size, num_workers=num_workers)
        self.dataloader_0_iterator = iter(self.dataloader_0)
        self.dataloader_1_iterator = iter(self.dataloader_1)
        self.device = device

    def get_images(self, clas):
        if clas == 0:
            try:
                return next(self.dataloader_0_iterator).to(self.device)
            except StopIteration:
                self.dataloader_0_iterator = iter(self.dataloader_0)
                return next(self.dataloader_0_iterator).to(self.device)
        else:
            try:
                return next(self.dataloader_1_iterator).to(self.device)
            except StopIteration:
                self.dataloader_1_iterator = iter(self.dataloader_1)
                return next(self.dataloader_1_iterator).to(self.device)
